...
- **Runtime** (`harness/runtime/`): `worker.py` polls `entity/tasks/`, picks next `status: todo` task by (priority → created → filename). When the queue goes empty and a cooldown has elapsed (default 30 min, `RESPONSIBILITIES_COOLDOWN_MINUTES`), the worker auto-enqueues a synthetic "Review responsibilities" planning task that asks the entity to spawn concrete tasks against `entity/responsibilities/`. Exceptions mark a task `blocked` and append a traceback.
...
- **Planning ticks are debounced in-memory.** The cooldown lives on the worker thread, so a restart can produce one extra tick — fine. Dedup also checks for a pending task tagged `planning-tick` to prevent stacking.
