...
- `list` — return all entries with cadence and last-run.
- `add` — register a new entry. Requires `name`, `responsibility`, and exactly one of `interval` / `cron`. Newly added entries wait one full period before first fire.
- `update` — change cadence or enabled state. Pass `interval: null` (or `cron: null`) to switch from one to the other.
