...
def run(**input) -> str:
    ...
    try:
        ...
        return "Error: `gws gmail +send` timed out after 60s."
    ...
    return _format_success(result.stdout, input)


def _format_success(stdout: str, params: dict) -> str:
    action = "Drafted" if params.get("draft") else "Sent"
    if params.get("dry_run"):
        action = "Dry-run validated"

    summary = f"{action}: to={params['to']!r} subject={params['subject']!r}"
    if cc := params.get("cc"):
        summary += f" cc={cc!r}"
